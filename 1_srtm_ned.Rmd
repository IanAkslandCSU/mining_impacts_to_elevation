...
```{r}
az_mine_proj <- st_transform(az_mine,4326)
mask_dem_diff <- mask(dem_diff,az_mine_proj)
maxValue(mask_dem_diff)
minValue(mask_dem_diff)
mapview(mask_dem_diff)
```
...
az_mine_buff <- st_buffer(az_mine,5000)
mapview(az_mine_buff)
mapview(az_mine)
```
...
```{r}
mask_dem_diff_inv <- mask(dem_diff,az_mine_proj,inverse = TRUE)
mapview(mask_dem_diff_inv)
```
...
```{r}
unmined_dem <- getValues(mask_dem_diff_inv)

mined_dem <- getValues(mask_dem_diff)
...
```{r}
geom_density_obj <- ggplot( data = mined_table,aes(x=dem_values, color = type)) + 
